...
let commandTest:  CommandTest;
let commandTest2:  CommandTest2;
...
describe('Bus Sync query/command app', function () {
    ...
    it('Should register a handler function and be called after the dispatch passing an option object.', function () {

        const options: object = {property1: "string1"};

        bus.registerHandler(CommandTest, function (command: CommandInstanceType, optionsGot: any): any {
            chai.assert.deepEqual(nameProperty, command.name);
    ...
    it('Should register two handlers functions and call both after the dispatch action.', function () {
        const nameProperty2: string = 'test->2';
    ...
    it('Should register a handler function using a string as the command name.', function () {
